...
const GrafikTrenSentimen: React.FC<GrafikTrenSentimenProps> = ({
  ...
  const availableFilters = React.useMemo(() => {
    if (validDates.length < 2) return ['harian'];
    const minDate = Math.min(...validDates);
    const maxDate = Math.max(...validDates);
    const diffDays = (maxDate - minDate) / (1000 * 60 * 60 * 24);

    const filters = ['harian'];
    if (diffDays >= 7) filters.push('mingguan');
    if (diffDays >= 28) filters.push('bulanan');
    if (diffDays >= 365) filters.push('tahunan');
    return filters;
  }, [validDates]);

  // Pastikan filter aktif selalu tersedia di dalam availableFilters
  React.useEffect(() => {
    if (!availableFilters.includes(filterWaktu)) {
      setFilterWaktu(availableFilters[availableFilters.length - 1] as any);
    }
  ...
  const aggregatedData = React.useMemo(() => {
    ...
    data.forEach(item => {
      ...
      if (!aggregated[key]) {
        aggregated[key] = { positif: 0, netral: 0, negatif: 0, timestamp };
      }
